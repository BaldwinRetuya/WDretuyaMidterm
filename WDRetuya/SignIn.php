         ...
         if ($user) {
            ...
            if ($locked_until && $locked_until > $now) {
                ...
                if (password_verify($password, $user['password'])) {
                    ...
                    // Reset attempt count
                    $conn->query("UPDATE signup SET attempt_count = 0 WHERE email = '$email'");
                    echo "<script>window.location.href = 'Outline.php';</script>";
                } else {
                    $attempt_count = $user['attempt_count'] + 1;
                    if ($attempt_count >= 3) {
                        $locked_until = new DateTime();
                        $locked_until->modify("+20 seconds");
                        $conn->query("UPDATE signup SET attempt_count = 0, locked_until = '" . $locked_until->format('Y-m-d H:i:s') . "' WHERE email = '$email'");
                        echo "<script>alert('Account locked due to too many failed attempts. Please try again later.');</script>";
                    } else {
                        $conn->query("UPDATE signup SET attempt_count = $attempt_count WHERE email = '$email'");
